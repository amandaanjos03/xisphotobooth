...
import { ChevronLeft, ChevronRight, Loader2, Maximize2, Minimize2, Pause, Play, RefreshCw, SwitchCamera } from "lucide-react";

type EventRow = { id: string; name: string; slug: string; bg_url: string | null; theme_slug: EventThemeSlug };
...
function LiveSlideshow() {
  ...
  const loadEvent = useCallback(async () => {
    ...
    const { data, error: eventError } = await supabase
      ...
      .eq("slug", slug)
      .maybeSingle();
    if (eventError) { setError("Não foi possível abrir este evento agora."); setLoading(false); return; }
    if (!data) { setError("Evento não encontrado."); setLoading(false); return; }
    const nextEvent = { ...data, theme_slug: normalizeEventTheme(data.theme_slug) } as EventRow;
    setEvent(nextEvent);
    seenRef.current.clear();
    setIdx(0);
    try { await loadPhotos(nextEvent.id, true); }
    catch { setError("As fotos não puderam ser carregadas. Tente novamente."); }
    setLoading(false);
  }, [loadPhotos, slug]);
  ...
  useEffect(() => {
    supabase.auth.getUser().then(async ({ data }) => {
      if (!data.user) return;
      const { data: events } = await supabase.from("events").select("name, slug").eq("owner_id", data.user.id).order("created_at", { ascending: false });
  ...
  useEffect(() => {
    ...
    const channel = supabase
      ...
      .on(
        ...
        (payload) => {
          ...
          refreshPhotoUrlsStrict([p]).then(([fresh]) => {
            if (!fresh) return;
            const image = fresh.media_type === "video" ? null : new Image();
            if (image) image.src = fresh.photo_url;
            setPhotos((current) => [fresh, ...current.filter((item) => item.id !== fresh.id)].slice(0, 200));
            setIdx(0);
    ...
    const poll = window.setInterval(() => {
      loadPhotos(event.id).then(() => setConnection((current) => current === "live" ? current : "polling")).catch(() => setConnection("polling"));
    }, 10000);
    return () => {
  ...
  useEffect(() => {
    if (!current) return;
    setPrevious((old) => old?.id === current.id ? old : photos[(idx - 1 + photos.length) % photos.length] ?? null);
  ...
  const move = (direction: number) => setIdx((value) => (value + direction + photos.length) % photos.length);
  ...
  if (!event) return (
    <div className="min-h-screen grid place-items-center bg-background px-5 text-center">
      {loading ? <Loader2 className="size-9 animate-spin text-primary" /> : (
        <div><h1 className="font-display text-3xl font-bold">{error ?? "Evento não encontrado"}</h1><Button className="mt-5 gap-2" onClick={loadEvent}><RefreshCw /> Tentar novamente</Button></div>
      )}
    </div>
  );
    ...
    >
      <header className="relative z-20 px-4 sm:px-6 py-3 flex items-center justify-between gap-3 bg-background/55 backdrop-blur-md border-b border-border/40">
        <div className="min-w-0">
          ...
          <div className="text-[11px] uppercase text-muted-foreground inline-flex items-center gap-2">
            <span className={`size-2 rounded-full ${connection === "live" ? "bg-accent animate-pulse" : "bg-muted-foreground"}`} />
            {connection === "live" ? "Ao vivo" : connection === "polling" ? "Atualizando" : "Conectando"} · {photos.length}
          </div>
        </div>
        <div className="flex items-center gap-1.5">
          {ownedEvents.length > 1 && (
            <label className="hidden sm:flex items-center gap-2 rounded-md border border-border bg-background/70 px-2 h-9">
              ...
              <select value={slug} onChange={(e) => changeEvent(e.target.value)} className="max-w-48 bg-transparent text-sm outline-none">
                {ownedEvents.map((item) => <option key={item.slug} value={item.slug}>{item.name}</option>)}
              </select>
            </label>
          )}
          <Button variant="ghost" size="icon" title={paused ? "Continuar" : "Pausar"} onClick={() => setPaused((value) => !value)}>{paused ? <Play /> : <Pause />}</Button>
          <Button variant="ghost" size="icon" title="Tela cheia" onClick={() => document.fullscreenElement ? document.exitFullscreen() : document.documentElement.requestFullscreen()}>{isFullscreen ? <Minimize2 /> : <Maximize2 />}</Button>
        </div>
      </header>
      ...
      <main className="relative flex-1 grid place-items-center p-3 sm:p-8 min-h-0">
        {loading ? (
          ...
          <div className="text-center"><p>{error}</p><Button variant="secondary" className="mt-4 gap-2" onClick={loadEvent}><RefreshCw /> Tentar novamente</Button></div>
        ) : !current ? (
          ...
          <>
            ...
            {photos.length > 1 && <>
              ...
              <Button variant="secondary" size="icon" className="absolute right-3 sm:right-6 z-20 rounded-full bg-background/65 backdrop-blur" onClick={() => move(1)} title="Próxima"><ChevronRight /></Button>
            </>}
          </>
        )}
      </main>
            ...
            {photos.slice(0, 20).map((p, i) => (
              <Button
                ...
                className={`shrink-0 size-12 sm:size-14 rounded-md p-0 overflow-hidden border-2 transition ${
                  i === idx ? "border-primary opacity-100" : "border-transparent opacity-55 hover:opacity-100"
                }`}
              >
                {p.media_type === "video" ? (
...
function mergePhotos(incoming: PhotoRow[], current: PhotoRow[]) {
  ...
  return Array.from(byId.values()).sort((a, b) => b.created_at.localeCompare(a.created_at)).slice(0, 200);
}
...
function LiveMedia({ media, className }: { media: PhotoRow; className?: string }) {
  ...
  return media.media_type === "video" ? (
    <video key={media.id} src={media.photo_url} autoPlay muted playsInline loop className={common} />
  ) : (
